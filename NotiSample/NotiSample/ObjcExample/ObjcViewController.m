...
- (void)subscribeToTopic {
    [AppBoxNotification.shared subscribeToTopic:@"topic_name" completion:^(BOOL success, NSError * _Nullable error) {
        if (error != nil) {
            NSLog(@"구독 실패 :: %@", error);
        } else {
            NSLog(@"구독 성공");
...
- (void)unsubscribeFromTopic {
    [AppBoxNotification.shared unsubscribeFromTopic:@"topic_name" completion:^(BOOL success, NSError * _Nullable error) {
        if (error != nil) {
            NSLog(@"해제 실패 :: %@", error);
        } else {
            NSLog(@"해제 성공");
